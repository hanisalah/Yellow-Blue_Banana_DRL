...
from Models.DuDQN_Model import DuDQN_Model
from ReplayBuffer.DuDQN_Replay import DuDQN_Replay
...
class DuDQN_Agent():
    ...
    def __init__(self, state_size, action_size, seed):
        ...
        # Q-Network
        self.qnetwork_local = DuDQN_Model(self.state_size, self.action_size, self.seed_int, fc1=self.config['fc1'], fc2=self.config['fc2']).to(self.device)
        self.qnetwork_target = DuDQN_Model(self.state_size, self.action_size, self.seed_int, fc1=self.config['fc1'],fc2=self.config['fc2']).to(self.device)
        self.optimizer = optim.Adam(self.qnetwork_local.parameters(), lr=self.config['lr'])

        # Replay memory
        self.memory = DuDQN_Replay(self.action_size, self.config['buffer_size'], self.config['batch_size'], self.seed)
    ...
    def update(self, params:dict):
        if len(params.keys())>0:
            ...
            if len(set(params) & {'fc1', 'fc2', 'normalize'})>0:
                self.qnetwork_local = DuDQN_Model(self.state_size, self.action_size, self.seed_int,
                                                 fc1=self.config['fc1'], fc2=self.config['fc2'], normalize=self.config['normalize']).to(self.device)
                self.qnetwork_target = DuDQN_Model(self.state_size, self.action_size, self.seed_int,
                                                  fc1=self.config['fc1'], fc2=self.config['fc2'], normalize=self.config['normalize']).to(self.device)
                self.optimizer = optim.Adam(self.qnetwork_local.parameters(), lr=self.config['lr'])
